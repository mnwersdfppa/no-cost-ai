...
Deno.serve(async (req: Request) => {
  ...
  if (action === "status") {
    await admin.rpc("refresh_bridge_rollout_gates");
    const [snapshot, rollout, controls, credentials, routes, nodes, queue] = await Promise.all([
      admin.from("bridge_readiness_snapshot").select("*").maybeSingle(),
      admin.from("bridge_rollout_summary").select("*").maybeSingle(),
    ...
    if (snapshot.error || rollout.error || controls.error || credentials.error || routes.error || nodes.error || queue.error) {
