...
async function markGate(gateKey: string, evidenceRef: string) {
  await admin.rpc("bridge_mark_completion_gate", {
    p_gate_key: gateKey,
    p_status: "pass",
...
Deno.serve(async (req: Request) => {
  ...
  if (action === "status") {
    ...
    const controlMap = Object.fromEntries(
      (controls.data ?? []).map((row) => [row.control_key, row.enabled]),
    );
    const safeControls =
      controlMap.paid_api_fallback === false &&
      controlMap.public_shell_execution === false &&
      controlMap.external_write_actions === false &&
      controlMap.phone_write_actions === false &&
      controlMap.telegram_single_poller_enforced === true &&
      controlMap.supabase_control_plane === true;
    ...
    if (correlationId && safeControls) {
      await markGate(
        "pi_authenticated_status",
        `edge:emergency-bridge:status:${correlationId}`,
      );
    }
    ...
    return reply({
      ...
      safe_controls_verified: safeControls,
  ...
  if (action === "heartbeat") {
    ...
    if (!nodeName || !nodeType || !status || !correlationId) {
    ...
    await admin.from("bridge_owner_registry").update({
      mode: "single_writer",
      status: status === "online" ? "active" : "degraded",
      notes: "Authenticated Raspberry Pi heartbeat received through emergency-bridge.",
      updated_at: new Date().toISOString(),
    }).eq("domain", "pi_execution");
    await markGate(
      "pi_heartbeat",
      `edge:emergency-bridge:heartbeat:${nodeName}:${correlationId}`,
    );
  ...
  if (action === "completion_receipt") {
    const gateKey = boundedString(body.gate_key, 1, 80);
    const evidenceSha256 = boundedString(body.evidence_sha256, 64, 64);
    const evidence = body.evidence && typeof body.evidence === "object" && !Array.isArray(body.evidence)
      ? body.evidence
      : {};
    if (!gateKey || !evidenceSha256 || !correlationId) {
      return safeError("invalid_completion_receipt", 400);
    }
    const { data, error } = await admin.rpc("bridge_accept_pi_completion_receipt", {
      p_user_id: user.id,
      p_gate_key: gateKey,
      p_correlation_id: correlationId,
    ...
    if (error) return safeError("completion_receipt_rejected", 400);
    return reply({
      ok: true,
      admission,
      gate: data,
