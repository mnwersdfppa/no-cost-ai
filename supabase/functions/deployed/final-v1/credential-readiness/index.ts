...
const CREDENTIAL_GROUPS: Record<string, string[]> = Object.freeze({
  ...
  telegram: ["TELEGRAM_BOT_TOKEN"], notion: ["NOTION_API_KEY", "NOTION_TOKEN"],
  github: ["GITHUB_TOKEN"], vercel: ["VERCEL_TOKEN", "VERCEL_API_TOKEN"],
});
...
function fail(code: string, status: number): Response {
  return reply({ ok: false, error: code, values_exposed: false, values_returned: false,
    prefixes_returned: false, hashes_returned: false, lengths_returned: false }, status);
}
...
Deno.serve(async (req: Request) => {
  ...
  const { data: admissionRows, error: admissionError } = await admin.rpc("bridge_admit_request", {
    p_user_id: user.id, p_action: "credential_readiness", p_execution_key: executionKey,
  });
  if (admissionError || !admissionRows?.[0]) return fail("admission_check_failed", 503);
  const admission = admissionRows[0];
  if (!admission.allowed) return reply({ ok: false, error: admission.reason, admission,
    values_exposed: false, values_returned: false, prefixes_returned: false,
    hashes_returned: false, lengths_returned: false }, admission.reason === "rate_limit_exceeded" ? 429 : 403);
  if (admission.duplicate) return reply({ ok: true, duplicate: true, admission,
    values_exposed: false, values_returned: false, prefixes_returned: false,
    hashes_returned: false, lengths_returned: false });

  await admin.rpc("bridge_record_runtime_key_selection", {
    p_user_id: user.id, p_function_name: "credential-readiness",
  ...
  for (const [integration, aliases] of Object.entries(CREDENTIAL_GROUPS)) {
    ...
    const { data: current } = await admin.from("bridge_credentials")
      ...
      .eq("integration", integration).maybeSingle();
    const priorPresence = current?.runtime_presence && typeof current.runtime_presence === "object" ? current.runtime_presence as JsonRecord : {};
    ...
    await admin.from("bridge_credentials").upsert({
      integration, canonical_secret_name: current?.canonical_secret_name ?? aliases[0],
      ...
      validation_status: protectedStatus ? current!.validation_status : present ? "unverified" : current?.validation_status ?? "not_present",
      validation_detail: protectedStatus ? current?.validation_detail : present
      ...
      required_scopes: current?.required_scopes ?? [], read_only_default: current?.read_only_default ?? true,
      runtime_presence: { ...priorPresence, supabase_edge_env: present }, last_validated_at: now, updated_at: now,
  ...
  await admin.rpc("bridge_record_event", {
    p_event_type: "credential_readiness_refresh", p_node_name: "raspberry-pi5",
    p_correlation_id: correlationId, p_severity: "info", p_outcome: "succeeded",
    p_detail: { actor_user_id: user.id, integrations_checked: results.length,
      runtime_server_key_type: adminKey.selectedType, values_exposed: false },
  });
  return reply({ ok: true, version: 6, admission, results,
    runtime_server_key_type: adminKey.selectedType,
    platform_managed_supabase_runtime: Boolean(SUPABASE_URL && adminKey.value),
    values_exposed: false, values_returned: false, prefixes_returned: false,
    hashes_returned: false, lengths_returned: false });
});
