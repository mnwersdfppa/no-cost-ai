-- Harden the existing token gateway ledger without changing provider routing.
...
drop policy if exists token_gateway_explicit_deny on public.token_gateway_usage;
create policy token_gateway_explicit_deny
  on public.token_gateway_usage
...
as
select
  user_id,provider,model,operation,date_trunc('day',created_at) as usage_day,
...
group by user_id,provider,model,operation,date_trunc('day',created_at);
...
create or replace function public.token_gateway_policy_gate(
  p_user_id uuid,p_provider text,p_model text,p_operation text
)
returns table(
  allowed boolean,reason text,approval_required boolean,daily_tokens bigint
)
language plpgsql
security definer
...
begin
  ...
  select enabled into v_paid_enabled from public.bridge_controls where control_key='paid_api_fallback';
  select enabled into v_external_writes from public.bridge_controls where control_key='external_write_actions';
  select coalesce(sum(total_tokens),0) into v_daily
  from public.token_gateway_usage
  where user_id=p_user_id and created_at>=date_trunc('day',now()) and status='succeeded';

  if lower(coalesce(p_provider,''))='openai' and coalesce(v_paid_enabled,false)=false then
    return query select false,'paid_api_fallback_disabled',true,v_daily;
    return;
  end if;

  if p_operation not in ('chat','summarize','extract','classify','plan','code_light','health') then
    return query select false,'operation_not_allowlisted',true,v_daily;
    return;
  end if;

  if p_operation='code_light' and coalesce(v_external_writes,false)=false then
    return query select false,'external_write_actions_disabled',true,v_daily;
    return;
  end if;

  if lower(coalesce(p_provider,'')) not in ('ollama','openrouter_free','phone_codex_oauth','openai') then
...
comment on function public.token_gateway_policy_gate(uuid,text,text,text) is
  'Fail-closed policy gate for future token-gateway revisions. Paid OpenAI remains disabled unless the explicit control is enabled.';
