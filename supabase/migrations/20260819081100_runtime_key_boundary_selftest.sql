-- Extend the Supabase emergency bridge self-test with the runtime server-key
-- class receipt boundary. No key value, prefix, hash, length or token is stored.
...
declare
  ...
  v_runtime_key_table boolean;
  v_runtime_key_grants integer;
  v_runtime_gate_pending boolean;
  v_status text;
  v_checks jsonb;
begin
  ...
  where (integration='supabase_platform'
         and operation in (
           ...
           'credential_readiness','resolve_route','runtime_key_receipt'
         )
         and enabled=true)
  ...
  where specific_schema='public'
    and routine_name in (
      ...
      'refresh_bridge_security_backlog','bridge_record_runtime_key_selection'
    )
    and grantee in ('anon','authenticated')
    and privilege_type='EXECUTE';

  select count(*) into v_capability_count from public.bridge_capability_registry;
  select count(*) into v_exportable_capabilities from public.bridge_capability_registry where credential_export_allowed=true;

  select count(*) into v_route_contracts
  from public.bridge_route_registry
  where (route_key='ollama.local' and capability='model_chat' and mode='free_only' and enabled=true and requires_control_key='local_ollama_route')
     or (route_key='openrouter.free' and capability='model_chat' and mode='free_only' and enabled=false and requires_control_key='openrouter_free_route')
     or (route_key='phone.codex_oauth' and capability='model_chat' and enabled=false and requires_control_key='phone_codex_route')
     or (route_key='openai.paid_api' and mode='disabled' and enabled=false and requires_control_key='paid_api_fallback');

  select count(*) into v_route_function_grants
  from information_schema.routine_privileges
  where specific_schema='public' and routine_name='bridge_resolve_route'
    and grantee='service_role' and privilege_type='EXECUTE';

  select to_regclass('public.bridge_runtime_key_selection') is not null into v_runtime_key_table;
  select count(*) into v_runtime_key_grants
  from information_schema.role_table_grants
  where table_schema='public' and table_name='bridge_runtime_key_selection'
    and grantee in ('anon','authenticated');

  select exists(
    select 1 from public.bridge_controls
    where control_key='modern_server_key_unification_verified' and enabled=false
  ) and exists(
    select 1 from public.bridge_security_backlog
    where item_key='runtime-key-selection:physical-pi-receipt' and status='open'
  ) into v_runtime_gate_pending;

  v_status:=case when
    v_required_tables=11 and v_rls_tables=11 and v_unsafe_grants=0
    and v_safe_controls=6 and v_required_policies=8
    and v_maintenance_cron and v_security_backlog_cron
    and v_unsafe_function_grants=0 and v_capability_count>=13
    and v_exportable_capabilities=0 and v_route_contracts=4
    and v_route_function_grants>=1 and v_runtime_key_table
    and v_runtime_key_grants=0 and v_runtime_gate_pending
  then 'pass' else 'fail' end;

  v_checks:=jsonb_build_object(
    'required_tables',jsonb_build_object('expected',11,'actual',v_required_tables,'pass',v_required_tables=11),
    'rls_tables',jsonb_build_object('expected',11,'actual',v_rls_tables,'pass',v_rls_tables=11),
    ...
    'required_policy_defaults',jsonb_build_object('expected',8,'actual',v_required_policies,'pass',v_required_policies=8),
    ...
    'runtime_key_receipt_table',jsonb_build_object('pass',v_runtime_key_table),
    'runtime_key_receipt_public_grants',jsonb_build_object('expected',0,'actual',v_runtime_key_grants,'pass',v_runtime_key_grants=0),
    'runtime_key_physical_gate_pending',jsonb_build_object('pass',v_runtime_gate_pending),
  ...
  values('supabase-first-emergency-bridge-v1.4',v_status,v_checks);
...
begin
  ...
  if v_result->>'status'<>'pass' then
    raise exception 'emergency bridge v1.4 self-test failed: %',v_result;
  end if;
end;
$$;
