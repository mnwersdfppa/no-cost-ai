...
function resolveAdminKey(): string {
  ...
  if (modern) {
    try {
      ...
      // Fall through only to the platform-managed legacy compatibility variable.
    }
  }
...
async function requirePiUser(req: Request) {
  ...
  if (data.user.app_metadata?.role !== "pi-gateway-client") throw safeError("pi_identity_required", 403);
  return data.user;
}

async function parseBody(req: Request): Promise<Record<string, unknown>> {
  const contentLength = Number(req.headers.get("content-length") ?? "0");
  if (Number.isFinite(contentLength) && contentLength > MAX_BODY_BYTES) throw safeError("payload_too_large", 413);
  try {
...
Deno.serve(async (req: Request) => {
  ...
  if (action === "status") {
    ...
    if (snapshot.error || controls.error || credentials.error || routes.error || nodes.error || queue.error) return safeError("readiness_read_failed", 503);
  ...
  if (action === "heartbeat") {
    ...
    await recordEvent("bridge_heartbeat", nodeName, correlationId, "info", "succeeded", { node_type: nodeType, status });
  ...
  if (action === "queue_status") {
    const { data, error } = await admin.from("openclaw_work_queue").select("status,priority,task_type,attempts,max_attempts,not_before,lease_until").order("priority", { ascending: false });
