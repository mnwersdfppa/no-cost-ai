...
function resolveAdminKey(): string {
  ...
  if (modern) {
    try {
      ...
      // Fall through only to the platform-managed legacy compatibility key.
    }
  }
...
Deno.serve(async (req: Request) => {
  if (!SUPABASE_URL || !ADMIN_KEY) return reply({ ok: false, error: "server_not_configured", values_exposed: false }, 503);
  ...
  return reply({
    ok: true,
    results,
    admin_key_source: Deno.env.get("SUPABASE_SECRET_KEYS") ? "managed_modern_secret_keys" : "managed_legacy_compatibility",
    platform_managed_supabase_runtime: Boolean(SUPABASE_URL && ADMIN_KEY),
