...
CORE_SOURCE="$SCRIPT_DIR/openclaw-recovery-worker.py"
LAUNCHER_SOURCE="$SCRIPT_DIR/openclaw-recovery-worker-v2.py"
FALLBACK_SOURCE="$SCRIPT_DIR/recover-openclaw-telegram-models.sh"
CORE_TARGET="$LIB_DIR/openclaw-recovery-worker-core.py"
...
[[ -f "$CORE_SOURCE" ]] || fail "worker_core_source_missing"
[[ -f "$LAUNCHER_SOURCE" ]] || fail "worker_launcher_source_missing"
[[ -f "$FALLBACK_SOURCE" ]] || fail "fallback_source_missing"

mkdir -p "$BIN_DIR" "$LIB_DIR" "$SECRETS_DIR" "$RUNTIME_DIR" "$USER_SYSTEMD_DIR"
chmod 700 "$BIN_DIR" "$LIB_DIR" "$SECRETS_DIR" "$RUNTIME_DIR"

install -m 0600 "$CORE_SOURCE" "$CORE_TARGET"
install -m 0700 "$LAUNCHER_SOURCE" "$WORKER_TARGET"
install -m 0700 "$FALLBACK_SOURCE" "$FALLBACK_TARGET"
python3 -m py_compile "$CORE_TARGET" "$WORKER_TARGET"
bash -n "$FALLBACK_TARGET"
OPENCLAW_RECOVERY_WORKER_CORE="$CORE_TARGET" "$WORKER_TARGET" --self-test >/dev/null
...
Environment=OPENCLAW_RECOVERY_WORKER_CORE=%h/.openclaw/lib/openclaw-recovery-worker-core.py
