...
SESSION_ENV = pathlib.Path(
    os.environ.get(
        "PI_WORK_QUEUE_ENV",
        SECRETS_DIR / "pi-work-queue.env",
    )
)
LOCK_FILE = RUNTIME_DIR / "external-scheduler-actuator.lock"
RECEIPT_FILE = RUNTIME_DIR / "external-scheduler-actuator-receipt.json"
DISABLE_SCRIPT = BIN_DIR / "openclaw-disable-project-schedulers"
...
def reload_session() -> None:
    ...
    ACCESS_TOKEN = os.environ.get(
        "PI_ACCESS_TOKEN",
        values.get("PI_ACCESS_TOKEN", ""),
    )
    REFRESH_TOKEN = os.environ.get(
        "PI_REFRESH_TOKEN",
        values.get("PI_REFRESH_TOKEN", ""),
    )
...
def refresh_session() -> bool:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "user-agent": "openclaw-external-scheduler-actuator/1",
        },
    )
    try:
...
def run_disable_script() -> dict[str, Any]:
    ...
    return {
        "handler": "fixed_project_scheduler_shutdown_v2",
        ...
        "rollback_script": str(
            BIN_DIR / "openclaw-rollback-project-schedulers"
        ),
...
def self_test() -> int:
    ...
    checks = {
        ...
        "fixed_handler": "run_disable_script()" in source,
